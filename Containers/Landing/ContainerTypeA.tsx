...
const ContainerTypeA = ({ height, data }: LandigCoponentProps) => {
            ...
            <div className=' row p-2  ' style={{ height: '70%' }} >
                ...
                <div className=' col-4  defaultHeight2Col mt-1'>
                    <div id={`${data.componentDetails && data.componentDetails[1] ? `CMPD${data.componentDetails[1]?.id}` : ''}`} className='  col-12 defaultBakground  defaultHeight1Col shadow myhoverStyle'
                        style={{ backgroundImage: images===true && data.componentDetails && data.componentDetails[1]?.attachments ? `url(${configure}/Global/GetFile/${ data?.componentDetails[1]?.attachments[0]?.path }?w=${getelemntWidth(`CMPD${data.componentDetails[0]?.id}`)}&h=${getelemntHeight(`CMPD${data.componentDetails[1]?.id}`)})` : '' }}>
                ...
                <div className=' col-4   defaultHeight2Col mt-1'>
                    <div id={`${data.componentDetails && data.componentDetails[3] ? `CMPD${data.componentDetails[3]?.id}` : ''}`} className='  col-12  defaultBakground  defaultHeight1Col shadow myhoverStyle'
